...
public class DockerSandbox {
	...
	//This file is created for each of the submissions 
	public File writeTmpFile() {
		try {
			...
			File parentDir = new File(System.getProperty("user.home") + System.getProperty("file.separator")
										+ "Cypher");
			...
			if (!parentDir.exists()) {
				if (parentDir.mkdir()) {
					tempFile = File.createTempFile("cypher_src-", ".tmp", parentDir);
				}
				else {
					System.err.println("<System(Cypher> Error: Failed to create parent directories for temporary"
											+ " file");
					throw new IOException();
				}
			}
			else {
				for (File oldFile : parentDir.listFiles()) {
					oldFile.delete();
				}
				tempFile = File.createTempFile("cypher_src-", ".tmp", parentDir);
			}
...
/*	
	public boolean copySource() {
		try {
			...
			String errorMessage = new String();
			String s = null;
			String container = new String(getCompiler(sub.language) + "-cypher");
			
			if (!getExtension(sub.language).contentEquals(".java")) {
				pb.command("docker", "exec", container, "-it", "/bin/sh");
				
				for (String com : pb.command()) {
				...
				if (setJavaClassPath(sub.code)) {
					pb.command("docker", "exec", container, "/bin/bash", "-c", "echo", "'",
							sub.code, "'" ,">",
							new String("/home/anonymous/" + getJavaClassName()
							+ getExtension(sub.language)), "\"");
